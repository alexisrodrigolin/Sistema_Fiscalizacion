...
class caja():
    ...
    def Resume(self, event=0):
        ...
        R_bonificacion_valor = tb.Entry(resume, style='Custom.TButton',
                                        font=("helvetica", int(25 * self.font_scale)), justify='center')
        R_total = tb.Label(resume, text='Total:', background='#F1EAD7', foreground='#847C67',
                           font=("helvetica", int(70 * self.font_scale)))
        R_total_valor = tb.Label(resume, text=f'$ {self.Bend.subtotal:.2f}',
                                 font=('Helvetica', int(70 * self.font_scale)), background='#F1EAD7',
                                 foreground='#847C67')
        R_bonificacion = tb.Label(resume, text='Bonificación %',
                                  font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                                  foreground='#847C67')
        R_efectivo = tb.Label(resume, text='+ Efectivo:',
                              font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                              foreground='#847C67')
        R_PagoElec = tb.Label(resume, text='+ Pago Virtual:',
                              font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                              foreground='#847C67')
        R_Tarjeta = tb.Label(resume, text='+ Tarjeta:',
                             font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                             foreground='#847C67')
        R_efectivo_valor = tb.Entry(resume, style='Custom.TButton',
                                    font=("helvetica", int(25 * self.font_scale)), justify='center')
        R_PagoElec_valor = tb.Entry(resume, style='Custom.TButton',
                                    font=("helvetica", int(25 * self.font_scale)), justify='center')
        R_tarjeta_valor = tb.Entry(resume, style='Custom.TButton',
                                   font=("helvetica", int(25 * self.font_scale)), justify='center')
        R_monto = tb.Label(resume, text='Total recibido:', background='#F1EAD7', foreground='#847C67',
                           font=("helvetica", int(30 * self.font_scale)))
        R_monto_valor = tb.Label(resume, text=f'$ 0.00',
                                 font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                                 foreground='#847C67')
        R_vuelto = tb.Label(resume, text='Vuelto:', background='#F1EAD7', foreground='#847C67',
                            font=("helvetica", int(30 * self.font_scale)))
        R_vuelto_valor = tb.Label(resume, text=f'$ 0.00',
                                  font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                                  foreground='#847C67')
    ...
    def mostrar_error(self, mensaje):
        ...
        label = tb.Label(error_window, text=f" ⚠ ⚠  {mensaje}  🚨",font=("Arial", int(30*self.font_scale)),  justify="center", foreground="#AB9F9F", background="#39384B")
        label.pack(pady=30)
