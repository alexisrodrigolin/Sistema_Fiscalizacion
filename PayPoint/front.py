...
class caja():
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.style.configure(
            ...
            foreground="black",
            borderwidth=0,
            font=("Helvetica", int(17 * self.font_scale)),
        )
        self.style.configure(
            "Custom.Treeview.Heading",
            background="#F1EAD7",
            foreground="black",
            font=("Helvetica", int(19 * self.font_scale), "bold"),
        )
        self.style.configure(
    ...
    def create_widget_menu(self):
        self.numberCaja = tb.Label(self.app, text="- Caja 1",
                                   font=("helvetica", int(35 * self.font_scale), "italic", "bold"),
    ...
    def create_widget_paypoint(self):
        ...
        self.total = tb.Label(self.app,
                              ...
                              font=("helvetica", int(80 * self.font_scale)),
                              relief="solid",
                              borderwidth=20,
                              anchor="w",
                              background='#F1EAD7')
        ...
        self.entrada = tb.Entry(self.app,
                                ...
                                font=("helvetica", int(50 * self.font_scale)),
                                justify="center")
    ...
    def menu(self, event=0):
        if self.Bend.subtotal == 0:
            ...
            for widget in self.app.winfo_children():
                widget.destroy()
            self.numberCaja = tb.Label(self.app, text="- Caja 1",
                                       font=("helveica", int(35 * self.font_scale), "italic", "bold"),
                                       background="#F1EAD7", anchor="center")
            self.numberCaja.grid(column=3, row=3)
            self.M_entrada()
        else:
    ...
    def ppunbind(self,event=0):
        for event_type in ("<Return>","<Escape>", "<+>", "<F2>", "<F3>", "<F4>", "<F5>", "<F6>", "<F7>", "<F8>", "<F9>", "<F10>"):
    ...
    def Resume(self, event=0):
        ...
        R_bonificacion_valor = tb.Entry(self.resume, style='Custom.TButton',
                                        font=("helvetica", int(25 * self.font_scale)), justify='center')
        R_total = tb.Label(self.resume, text='Total:', background='#F1EAD7', foreground='#847C67',
                           font=("helvetica", int(70 * self.font_scale)))
        R_total_valor = tb.Label(self.resume, text=f'$ {self.Bend.subtotal:.2f}',
                                 font=('Helvetica', int(70 * self.font_scale)), background='#F1EAD7',
                                 foreground='#847C67')
        R_bonificacion = tb.Label(self.resume, text='Bonificación %',
                                  font=('Helvetica', int(30 * self.font_scale)), background='#F1EAD7',
                                  foreground='#847C67')
        ...
        R_monto = tb.Label(self.resume, text='Total recibido:', background='#F1EAD7', foreground='#847C67',
                           font=("helvetica", int(30 * self.font_scale)))
    ...
    def mostrar_error(self, error, state=0):
        ...
        label = tb.Label(error_window, text=f"⚠︎  {message}",font=("Arial", int(30*self.font_scale)),  justify="center", foreground="#AB9F9F", background="#39384B")
        label.pack(pady=30)
        if not state:tb.Label(error_window, text= 'Contáctese con el técnico',font=("Arial", int(15*self.font_scale)),  justify="center", foreground="red", background="#39384B").pack(pady=30)
        if state==4:
    ...
    def Sales(self):
        ...
        self.SalesBind()
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Caja:", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.05, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Fecha: ", font=("Arial", int(30*self.font_scale))).place(relx=0.35, rely=0.05, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Monto Ac.", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.15, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Facturado SF", font=("Arial", int(30*self.font_scale))).place(relx=0.3, rely=0.15, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Total", font=("Arial", int(30*self.font_scale))).place(relx=0.55, rely=0.15, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Efectivo:", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.25, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.3, rely=0.25, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.55, rely=0.25, relwidth=0.2, relheight=0.05)

        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Pago Elect:", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.35, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.3, rely=0.35, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.55, rely=0.35, relwidth=0.2, relheight=0.05)

        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Tarjeta:", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.45, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.3, rely=0.45, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.55, rely=0.45, relwidth=0.2, relheight=0.05)

        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="Total:", font=("Arial", int(30*self.font_scale))).place(relx=0.05, rely=0.55, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.3, rely=0.55, relwidth=0.2, relheight=0.05)
        tb.Label(self.sales, background= '#F1EAD7', foreground= '#847C67', text="$4000", font=("Arial", int(30*self.font_scale))).place(relx=0.55, rely=0.55, relwidth=0.2, relheight=0.05)
    ...
    def Auditoria(self, event=0):
        ...
        tb.Label(self.auditoria, text= "Por Fecha", background= '#F1EAD7', foreground= '#847C67',font=("Arial", int(30*self.font_scale))).place(relx=0.245, rely=0.1016,)
        tb.Label(self.auditoria, text= "Por Num de Z", background= '#F1EAD7', foreground= '#847C67',font=("Arial", int(30*self.font_scale))).place(relx=0.5586, rely=0.1016, )
        ...
        S_fecha= tb.Entry(self.auditoria, style="Custom.TButton",font=("helvetica", int(25 * self.font_scale)), justify='center')
        F_fecha= tb.Entry(self.auditoria, style="Custom.TButton",font=("helvetica", int(25 * self.font_scale)), justify='center')
        ...
        S_numZ= tb.Entry(self.auditoria, style="Custom.TButton",font=("helvetica", int(25 * self.font_scale)), justify='center')
        F_numZ= tb.Entry(self.auditoria, style="Custom.TButton",font=("helvetica", int(25 * self.font_scale)), justify='center')
